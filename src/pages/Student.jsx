...
const SKILL_EVAL = {
  ...
  1: {
    C: { canDo: ["Збирає прості схеми на Breadboard", "Розуміє полярність та читає прості схеми"], workOn: ["Вивчити закон Ома та розраховувати резистор", "Зрозуміти потенціометр та подільник напруги"] },
...
export default function Student() {
  ...
  async function reassign(newGroup) {
    if (newGroup === student.group) return;
    setReassigning(true);
    ...
    setStudent(s => ({ ...s, group: newGroup, manual: true }));
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: "system-ui, sans-serif" }}>
      ...
      {/* Header */}
      <div style={{ background: "#fff", borderBottom: "0.5px solid #e5e7eb", padding: "12px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={() => navigate("/dashboard")} style={{ background: "none", border: "none", color: "#9ca3af", cursor: "pointer", fontSize: 13, padding: 0 }}>← Назад</button>
          <div style={{ width: 1, height: 18, background: "#e5e7eb" }} />
          <div style={{ fontWeight: 700, fontSize: 15, color: "#111827", fontFamily: "monospace" }}>{student.name}</div>
          {student.manual && <div style={{ fontSize: 11, color: "#10b981", fontWeight: 600 }}>↑ Переведено вручну</div>}
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <span style={{ fontSize: 11, color: "#9ca3af", marginRight: 4 }}>Перевести до:</span>
          {["A", "B", "C"].map(k => {
            const info = GRP[k];
            const active = student.group === k;
            return (
              <button key={k} onClick={() => reassign(k)} disabled={reassigning}
                style={{ padding: "5px 14px", borderRadius: 6, fontWeight: active ? 700 : 500, fontSize: 13, border: active ? `1.5px solid ${info.color}` : "1px solid #e5e7eb", background: active ? info.light : "transparent", color: active ? info.dim : "#9ca3af", cursor: active ? "default" : "pointer", transition: "all .15s", opacity: reassigning ? 0.5 : 1 }}>
                {k}{active ? " ✓" : ""}
              </button>
            );
          })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", minHeight: "calc(100vh - 53px)" }}>

        {/* Sidebar */}
        <div style={{ borderRight: "0.5px solid #e5e7eb", padding: "24px 20px", background: "#fff" }}>
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 10, color: "#9ca3af", letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>Поточна група</div>
            <div style={{ fontSize: 56, fontWeight: 800, color: g.color, lineHeight: 1, fontFamily: "monospace" }}>{student.group}</div>
            <div style={{ fontSize: 13, color: "#6b7280", fontWeight: 600, marginTop: 2 }}>{g.label} · {g.tag}</div>
          </div>
          <div style={{ borderTop: "0.5px solid #e5e7eb", paddingTop: 16 }}>
            <div style={{ fontSize: 10, color: "#9ca3af", letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>Загальний бал</div>
            <div style={{ fontSize: 28, fontWeight: 800, fontFamily: "monospace", color: g.color, marginBottom: 16 }}>
              {student.total}<span style={{ fontSize: 14, color: "#9ca3af", fontWeight: 400 }}>/20</span>
            </div>
            <div style={{ fontSize: 10, color: "#9ca3af", letterSpacing: 1, textTransform: "uppercase", marginBottom: 12 }}>Навички</div>
